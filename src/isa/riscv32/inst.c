...
void ftrace_jump(bool is_jalr,  vaddr_t current_pc, vaddr_t next_pc, int rd, word_t imm)
{
    if (is_jalr && rd == 0 && imm == 0)
    {
        // ret
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm)&(~((vaddr_t)1)); R(rd) = s->pc + 4; IFDEF(CONFIG_FTRACE, ftrace_jump(true, s->pc, s->dnpc, rd, imm)););
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; R(rd) = s->pc + 4;IFDEF(CONFIG_FTRACE, ftrace_jump(false, s->pc, s->dnpc, rd, imm)););
